...
function evaluate(ct::Vector{Int8}, a::Int8, b::Int8, indexes::Matrix{Union{Nothing,Int8}})
    n = Int8(size(indexes)[1])
    a == Int8(0) && return Int8(0)
    b == Int8(0) && return Int8(0)
    a == n-Int8(1) && return b
    b == n-Int8(1) && return a
    return a < b ? ct[indexes[a+Int8(1), b+Int8(1)]] : ct[indexes[b+Int8(1), a+Int8(1)]]
end
...
We could represent the Cayley table for (C_N, ⋅) with an N × N matrix, but:
 ...
 - since 1 is an identity element and 0 an absorbing element, we don't need
   to represent the first and last rows/columns, so we just need a (N-2)^2 matrix.
...
Also, since the values of N we can evaluate are very small (way less than 127) we can use
Int8 to save up space.
"""
struct FiniteFLewChain{N}
    cayleytable::Vector{Int8}
    ...
    if n < Int8(3)
        ...
        for i in Int8(0):n-Int8(1)
            for j in Int8(0):n-Int8(1)
                indexes[i+Int8(1), j+Int8(1)] = (n-Int8(2))*(i-Int8(1))+j-(i*(i-Int8(1)))÷Int8(2)
            end
        end
        return FiniteFLewChain{n}.([generateflewchains(n, indexes)...]; safe=true)
    end
end
